...
public class RestoreTrashedTodoHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_PreservesTimeSpentSeconds()
    {
        var db = await TestDatabase.CreateAsync();
        var add = new AddTodoHandler(db);
        var startTimer = new StartTimerHandler(db);
        var stopTimer = new StopTimerHandler(db);
        var delete = new DeleteTodoHandler(db);
        var getTrashed = new GetTrashedTodosHandler(db);
        var restore = new RestoreTrashedTodoHandler(db);
        var getTodos = new GetTodosHandler(db);

        var id = await add.HandleAsync("Tracked task");
        await startTimer.HandleAsync(id);
        await Task.Delay(1100);
        await stopTimer.HandleAsync(id);
        var loggedSeconds = (await getTodos.HandleAsync()).Single().TimeSpentSeconds;
        ...
        var restored = (await getTodos.HandleAsync()).Single();
        Assert.Equal(loggedSeconds, restored.TimeSpentSeconds);
        Assert.True(restored.TimeSpentSeconds >= 1);
    }
}
